...
void control_with_vectorial_command(){
    ...
    if( motor_is_tared() ){
        ...
        if(!manual_speed){
            ...
            speed_load = ((((limited_theta_error/16)*(k_pos_i/32768))/32768)*((8947848)/256));
            speed_i__acc_sum = (speed_i/1 + speed_load/32);
        ...
        voltage_load = ((((limited_speed_error/1024)*(k_speed_i/32768))/32768)*((8947848)/256));
        voltage_i__acc_sum = (voltage_i/1 + voltage_load/32);
...
TERMINAL_COMMAND(speed_csg, "Set speed consign")
{
    if (argc == 1) {
        ...
        if(speed_csg_input < -18.0) speed_csg_input = -18.0;
        if(speed_csg_input > 18.0) speed_csg_input = 18.0;
        speed_csg = (int) (speed_csg_input * 67108864);
        // end of automatic code - generated by integer_calculus.py
    } else {
        if(argc==0){
            terminal_io()->println((double)speed_csg/67108864);
            return;
        }
        terminal_io()->println("Usage: speed_csg [ -18.0 - 18.0 ]");
    }
}
