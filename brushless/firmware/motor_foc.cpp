...
static int user_pwm = 0*16777216;
...
void control_motor_with_vectorial( int theta ){
    ...
    phase_pwm_w = ((alpha_user_pwm/65536)*((phase_voltage_w/1 + (-min_voltage)/1)/256));
    phase_pwm_v = ((alpha_user_pwm/65536)*((phase_voltage_v/1 + (-min_voltage)/1)/256));
    phase_pwm_u = ((alpha_user_pwm/65536)*((phase_voltage_u/1 + (-min_voltage)/1)/256));
    // end of automatic code - generated by integer_calculus.py
...
TERMINAL_COMMAND(user_pwm, "Force the phase to use a given pwm")
{
    if (argc == 1) {
        ...
        if(user_pwm_input > 100) user_pwm_input = 100;
        user_pwm = (int) (user_pwm_input * 16777216);
        // begin of automatic code - generated by motor_foc.py
    } else {
        if(argc==0){
            terminal_io()->println(((double)user_pwm)/16777216);
            return;
        }
